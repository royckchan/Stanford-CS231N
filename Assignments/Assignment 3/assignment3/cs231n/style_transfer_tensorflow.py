...
def tv_loss(img, tv_weight):
    ...
    diff_vertical = (tf.gather(img, tf.range(H-1), axis=0)
            - tf.gather(img, tf.range(1, H), axis=0))
    diff_horizontal = (tf.gather(img, tf.range(W-1), axis=1)
            - tf.gather(img, tf.range(1, W), axis=1))
    loss = tv_weight * (
            tf.math.reduce_sum(tf.math.square(diff_vertical)) +
            tf.math.reduce_sum(tf.math.square(diff_horizontal)))
    return loss
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    style_loss = 0.
    for idx, gram_source, style_weight in zip(
            style_layers, style_targets, style_weights):
        features = feats[idx]
        gram = gram_matrix(features)
        style_loss += style_weight * tf.math.reduce_sum(
                tf.math.square(gram - gram_source))
    return style_loss
...
def content_loss(content_weight, content_current, content_original):
    ...
    content_loss = content_weight * tf.math.reduce_sum(
            tf.math.square(content_current - content_original))
    return content_loss
