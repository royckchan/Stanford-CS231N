...
def discriminator_loss(logits_real, logits_fake):
    ...
    # N_real can be smaller than N_fake at the end of each epoch
    N_real = tf.shape(logits_real)[0].numpy()
    N_fake = tf.shape(logits_fake)[0].numpy()
...
def generator_loss(logits_fake):
    ...
    N = tf.shape(logits_fake)[0].numpy()
    bce = tf.keras.losses.BinaryCrossentropy(from_logits=True)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = (0.5 * tf.math.reduce_mean(tf.math.square(scores_real - 1))
            + 0.5 * tf.math.reduce_mean(tf.math.square(scores_fake)))
...
def dc_generator(noise_dim=NOISE_DIM):
    ...
    layers = [
            ...
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Reshape((7, 7, 128), input_shape=(7*7*128, )),
