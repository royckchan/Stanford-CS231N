...
def compute_saliency_maps(X, y, model):
    ...
    with tf.GradientTape() as tape:
        tape.watch(X)
        scores = model(X)
        scores_correct_class = tf.gather_nd(scores, tf.stack((tf.range(N), y), axis=1))
    gradient = tape.gradient(scores_correct_class, X) 
    saliency = gradient.numpy()
...
def make_fooling_image(X, target_y, model):
    ...
    class_predicted = tf.math.argmax(scores[0]).numpy()
    while class_predicted != target_y:
        with tf.GradientTape() as tape:
            tape.watch(X_fooling)
            scores = model(X_fooling)
            target_score = scores[0, target_y]
            class_predicted = tf.math.argmax(scores[0]).numpy()
        gradient = tape.gradient(target_score, X_fooling)
        dX = learning_rate * gradient / tf.norm(gradient)
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
        ...
        scores = model(X)
        target_score = scores[0, target_y]
        reg = l2_reg * tf.math.square(tf.norm(X))
        objective = target_score - reg
    gradient = tape.gradient(objective, X)
    X.assign_add(learning_rate * gradient)
