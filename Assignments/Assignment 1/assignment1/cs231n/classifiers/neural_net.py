...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        Y[range(N), y] = 1.0
        grad_scores = probs - Y
        grad_W2 = 1/N * np.matmul(h.T, grad_scores) + 2 * reg * W2
        grad_b2 = 1/N * np.sum(grad_scores, axis=0)
        grad_Z1 = np.matmul(grad_scores, W2.T) * (h > 0)
        grad_W1 = 1/N * np.matmul(X.T, grad_Z1) + 2 * reg * W1
